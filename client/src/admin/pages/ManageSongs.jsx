...
import { getSongs, deleteSong } from "../../api/services/songService";
...
function ManageSongs() {
  ...
  const handleDeleteTrigger = (song) => {
        ...
        <div className="overflow-hidden rounded-3xl border border-brand-gold/20 bg-brand-surface shadow-soft">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-brand-gold/20 bg-brand-cream/15 text-xs font-bold uppercase tracking-wider text-brand-green-950">
              ...
              <tbody className="divide-y divide-brand-gold/10 text-sm text-brand-green-950">
                {songs.map((song) => (
                  <tr key={song._id} className="hover:bg-brand-cream/5 transition">
                    ...
                    <td className="px-6 py-4 font-mono text-xs">{song.year || "N/A"}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <button
                          onClick={() => handleDeleteTrigger(song)}
